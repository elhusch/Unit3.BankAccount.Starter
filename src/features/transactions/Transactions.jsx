...
/**
 * Allows users to deposit to, withdraw from, and transfer money
 * from their account.
 */
export default function Transactions() {
  // TODO: Get the balance from the Redux store using the
  // useSelector hook

  const balance = useSelector((state) => state.transactions.balance); //
  //issue I had with most of the code was needing to insert ".transactions"
  const dispatch = useDispatch();

  const [amountStr, setAmountStr] = useState("0.00");
  const [recipient, setRecipient] = useState("");

  /** Dispatches a transaction action based on the form
   * submission. */
  const onTransaction = (e) => {
    e.preventDefault();

    // This changes depending on which button the user clicked
    //to submit the form.
    ...
    // TODO: Dispatch the appropriate transaction action
    //based on `action`
    if (action === "transfer") {
      // The `transfer` action is dispatched with a payload
      //containing
